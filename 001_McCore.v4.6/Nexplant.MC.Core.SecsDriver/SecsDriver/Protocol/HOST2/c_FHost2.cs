...
--  Description     : FAMate Core FaSecsDriver Host2 Parser Class
--  History         : Created by spike.lee at 2011.11.08                    
----------------------------------------------------------------------------------------------------------*/
using System;
...
namespace Nexplant.MC.Core.FaSecsDriver
{
    internal static class FHost2
    {
            ...
            )
        {
            ...
            FXmlNodeList fXmlNodeListHdmg = null;
            FXmlNode fXmlNodeTempHmg = null;
            ...
            try
            {
                ...
                if (fXmlNodeHsn == null)
                {
                    ...
                    // Not Define Session
                    // ***
                    fXmlNodeHmgl = FSecsDriverLogCommon.createXmlNodeHMGL(fScd.fScdCore.fXmlDoc);
                ...
                if (!ignoreStructure)
                {
                    ...
                    if (fXmlNodeRepHmg == null)
                    {
                        ...
                        // --
                        fXmlNodeListHmg = fScd.fXmlNode.selectNodes(xpath);
                        // --
                ...
                {
                    // --
                    // Add by Jeff.Kim 2018.02.21
                    // 구조를 무시하는 옵션 일 경우에는 해당 Command로 메세지만 찾아 온다. 
                    #region 구조 무시
                    ...
                    fXmlNodeTempHmg = fScd.fXmlNode.selectSingleNode(xpath);
                    if (fXmlNodeTempHmg != null)
                    {
                        fXmlNodeHmg = fXmlNodeTempHmg.clone(true);
                    }
                ...
                if (fXmlNodeHmg == null)
                {
                    fXmlNodeHmgl = FSecsDriverLogCommon.createXmlNodeHMGL(fScd.fScdCore.fXmlDoc);
                    setHostMessageLogInfo(fXmlNodeHmgl, fHdv.fXmlNode, fXmlNodeHsn, fHdmg.machineId, fHdmg.sessionId, fHdmg.command, 0, fHdmg.fHostMessageType, fHdmg.tid);
                    fXmlNodeHmgl.set_attrVal(FXmlTagSMGL.A_Name, FXmlTagSMGL.D_Name, "Undefined");
                    // --
                    generateHdmgToHmg(fXmlNodeHmgl, fHdmg);
                    // --
                    fResultCode = FResultCode.Warninig;
                ...
                else
                {
                    // --
                    // Log Item 을 추가 한다.
                    fXmlNodeListHdmg = fHdmg.fXmlNode.selectNodes(FXmlTagHIT.E_HostItem);
                    for (int i = 0; i < fXmlNodeListHdmg.count; i++)
                    {
                        fXmlNodeHmgl.appendChild(fXmlNodeListHdmg[i].clone(true));
                    }

                    // --
                    // Add by Jeff.Kim 2018.02.21
                    // 구조 무시의 경우 받은 메세지의 Item 만 Clone하여 추가 해준다. 
                    updateHitUniqueId(
                        fXmlNodeHmgl,
                        fXmlNodeHmg.fChildNodes
                        );
                }
        ...
        // -- 
        // Add by Jeff.Kim 2018.02.22
        // Host Driver Data Message에서 받은 Host Item의 Unique ID 값을 모델링 된 Item Name 기준으로 Update한다.
        private static void updateHitUniqueId(            
            FXmlNode fXmlNodeHmgl,
            FXmlNodeList fXmlNodeHitList
            )
        {
            // --
            string xpath = "//" + FXmlTagHIT.E_HostItem + "[@" + FXmlTagHIT.A_Name + "='{0}']";
            FXmlNode xTempHitNode = null;
            ...
            try
            {
                ...
                foreach (FXmlNode x in fXmlNodeHitList)
                {
                    fFormat = FEnumConverter.toFormat(x.get_attrVal(FXmlTagHIT.A_Format, FXmlTagHIT.D_Format));
                    ...
                    {
                        // --
                        xTempHitNode = fXmlNodeHmgl.selectSingleNode(string.Format(xpath, x.get_attrVal(FXmlTagHIT.A_Name, FXmlTagHIT.D_Name)));
                        if (xTempHitNode != null)
                        {
                            xTempHitNode.set_attrVal(FXmlTagHIT.A_UniqueId, x.get_attrVal(FXmlTagHIT.A_UniqueId, FXmlTagHIT.D_UniqueId));
                        }
                    }
                }
                    ...
                    if (fPattern == FPattern.Fixed)
                    {
                        ...
                        for (int i = 0; i < fixLen; i++)
                        {
                            ...
                            else
                            {
                                ...
                                if (fFormat1 == FFormat.Ascii || fFormat1 == FFormat.JIS8 || fFormat1 == FFormat.A2 || fFormat1 == FFormat.Char || valLen == 1)
                                {
            ...
            )
        {
            ...
            try
            {
                fXmlNodeHmg = fScd.fXmlNode.selectSingleNode(
            ...
            )
        {
            ...
            try
            {
                ...
                fXmlNodeListHcn = fScd.fXmlNode.selectNodes(xpath);
            ...
            )
        {
            ...
            try
            {
                ...
                foreach (FXmlNode fXmlNodeHcn in fXmlNodeListHcn)
                {
                    ...
                    {
                        compareCondition(fScd, fXmlNodeHmgl, fXmlNodeHep, ref result);
                    }
            ...
            )
        {
            ...
            try
            {
                ...
                if (fExpressionType == FExpressionType.Bracket)
                {
                    ...
                    {
                        compareCondition(fScd, fXmlNodeHmgl, fXmlNodeChild, ref newResult);
                    }
                ...
                {
                    if (fOperandFormat == FFormat.Ascii || fOperandFormat == FFormat.A2 || fOperandFormat == FFormat.JIS8 || fOperandFormat == FFormat.Char)
                    {
