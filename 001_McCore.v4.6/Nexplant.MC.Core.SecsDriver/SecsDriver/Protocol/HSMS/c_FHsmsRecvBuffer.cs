...
--  Description     : FAMate Core FaSecsDriver HSMS Receive Buffer Class 
            ...
            try
            {
                while (m_data.Count > 0)
                {
                    if (!m_isLengthComp)
                    {
                        ...
                        // --
                        m_length = FByteConverter.toUInt32((byte[])m_data.GetRange(0, 4).ToArray(typeof(byte)), true);
                        // --
                    ...
                    else if (!m_isHeaderComp)
                    {
                        ...
                        // --
                        m_sessionId = FByteConverter.toUInt16((byte[])m_data.GetRange(0, 2).ToArray(typeof(byte)), true);
                        // m_sessionId = (UInt16)(m_sessionId & 0x8000);   // R-Bit 제거 (SECS-I 호환)
                        m_byte2 = (byte)m_data[2];
                        m_byte3 = (byte)m_data[3];
                        m_ptype = (byte)m_data[4];
                        m_stype = (byte)m_data[5];
                        m_systemBytes = FByteConverter.toUInt32((byte[])m_data.GetRange(6, 4).ToArray(typeof(byte)), true);
                        // --
                    ...
                    else if (!m_isBodyComp)
                    {
                        ...
                        // --
                        m_body = (byte[])m_data.GetRange(0, (int)(m_length - 10)).ToArray(typeof(byte));
                        // --
