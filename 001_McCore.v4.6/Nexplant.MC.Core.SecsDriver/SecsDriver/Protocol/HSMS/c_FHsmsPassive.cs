...
--  Description     : FAMate Core FaSecsDriver HSMS Passive Class 
    ...
    internal class FHsmsPassive : FBaseHsms
    {
            ...
            )
        {
            try
            {
                ...
                m_fTmrHsmsPassive2CleanUp.start(1000 * 10);     // 10초
            ...
            )
        {
            try
            {
                m_fTcpListener = new FTcpListener(
                    ...
                    this.fHsmsProtocol.fSecsDevice.localPort, 
                    1
                    );
                // --
                m_fTcpListener.TcpListenerAcceptCompleted += new FTcpListenerAcceptCompletedEventHandler(m_fTcpListener_TcpListenerAcceptCompleted);
            ...
            )
        {
            try
            {
                m_fMainSync.wait();
                // --
                foreach (FHsmsPassive2 hp2 in m_fHsmsPassive2List)
                {
                    hp2.pauseProtocol();
                }
            }
            ...
            )
        {
            ...
            try
            {
                ...
                {
                    if (m_fHsmsPassive2List[i].fDeviceState == FDeviceState.Closed)
                    {
                        indexList.Add(i);
                    }                    
                }
        ...
        private void procAccepted2(
            FTcpClient fTcpClient
            )
        {
            try
            {
                m_fHsmsPassive2List.Add(new FHsmsPassive2(this.fHsmsProtocol, fTcpClient));
            }
            catch (Exception ex)
            {
                procDeviceErrorRaised(ex);
            ...
            try
            {
                ...
                else
                {
                    if (state is FSecsDeviceControlMessageSentLog)
                    {
                        ...
                        {
                            if (((FSecsDeviceControlMessageSentLog)state).byte3 == 0)
                            {
                                procStateSelected();
                            }
                        }
            ...
            )
        {
            ...
            {                
                ...
                if (fScdCore.fConfig.enabledEventsOfSecsDeviceControlMessage)
                {
                    ...
                    // --
                    this.fEventPusher.pushSecsDeviceControlMessageReceivedEvent(this.fSecsDevice, fResultCode, this.getSelectStatusMessage(status), fLog);
                }
            ...
            )
        {
            ...
            try
            {
                if (fScdCore.fConfig.enabledEventsOfSecsDeviceControlMessage)
                {
                    ...
                    // --                
                    this.fEventPusher.pushSecsDeviceControlMessageReceivedEvent(this.fSecsDevice, FResultCode.Warninig, this.getRejectReasonMessage(1), fLog);
                }
            ...
            )
        {
            ...
            {                
                ...
                {
                    fResultCode = FResultCode.Warninig;
                    resultMessage = string.Format(FConstants.err_m_0015, "Session ID of the Linktst.Rsp");
                }
                else if (m_fTranLinktestReq.systemBytes != m_fRecvBuf.systemBytes)
                {
                    fResultCode = FResultCode.Warninig;
                    resultMessage = string.Format(FConstants.err_m_0015, "System Bytes of the Linktest.Rsp");
                }
            ...
            )
        {
            ...
            try
            {
                ...
                // ***
                // Linktest Transaction Timeout Check
                // ***
