...
--  Description     : FAMate Core FaSecsDriver Auto Cycle Agent Class 
...
namespace Nexplant.MC.Core.FaSecsDriver
{
    internal class FAutoCycleAgent : IDisposable
    {
        ...
        public void collectSecsAutoCycleData(            
            ...
            )
        {
            const string Query =
                ...
                "[" +
                "@" + FXmlTagSTN.A_UsedAutoCycle + "='{0}' and " +
            ...
            try
            {
                fXmlNodeListStn = m_fSecsDriver.fXmlNode.selectNodes(string.Format(Query, FBoolean.True, fSecsDevice.uniqueIdToString));
                if (fXmlNodeListStn.count == 0)
                {
                ...
                {
                    uniqueId = fXmlNodeStn.get_attrVal(FXmlTagSTN.A_UniqueId, FXmlTagSTN.D_UniqueId);
                    period = int.Parse(fXmlNodeStn.get_attrVal(FXmlTagSTN.A_AutoCyclePeriod, FXmlTagSTN.D_AutoCyclePeriod));
                    fAutoCycleAction = FEnumConverter.toAutoCycleAction(fXmlNodeStn.get_attrVal(FXmlTagSTN.A_AutoCycleAction, FXmlTagSTN.D_AutoCycleAction));
                    ...
                    if (m_autoCycleKeys.Contains(uniqueId))
                    {
                        ...
                        else
                        {
                            ...
                            // --
                            newAutoCycleKeys.Add(uniqueId);
                            newAutoCycleList.Add(uniqueId, new FAutoCycleData(uniqueId, period, fAutoCycleAction, fXmlNodeStn));
                        }                        
                    }
                    else
                    {
            ...
            )
        {
            ...
            try
            {
                fXmlNodeListHtn = m_fSecsDriver.fXmlNode.selectNodes(string.Format(Query, FBoolean.True, fHostDevice.uniqueIdToString));
