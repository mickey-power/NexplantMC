...
--
--  Program Id      : c_FTelnetActive.cs
--  Creator         : byjeon
--  Create Date     : 2012.01.27
--  Description     : FAMate Core FaSecsDriver TELNET Active Class 
--  History         : Created by byjeon at 2012.01.27
...
{
    internal class FTelnetActive : FBaseTelnet
    {
        ...
        // -- 
        private FTelnetClient m_fTelnetClient = null;
        private bool m_isLocalTransmitBinary = false;
        private bool m_isRemoteTransmitBinary = false;
        // --
        ...
        public FTelnetActive(
            FTelnetProtocol fTelnetProtocol
            )
            : base(fTelnetProtocol)
        {
            init();
        }
            ...
            )
        {
            try
            {
                ...
                m_fThdMain = new FThread("FTelnetActiveMainThread");
            ...
            )
        {
            try
            {
                m_fTcpClient = new FTcpClient(
                    this.fTelnetProtocol.fSecsDevice.localIp,
                    this.fTelnetProtocol.fSecsDevice.remoteIp,
                    this.fTelnetProtocol.fSecsDevice.remotePort
                    );
                // --
                m_fTcpClient.TcpClientStateChanged += new FTcpClientStateChangedEventHandler(m_fTcpClient_TcpClientStateChanged);
                m_fTcpClient.TcpClientDataReceived += new FTcpClientDataReceivedEventHandler(m_fTcpClient_TcpClientDataReceived);
            ...
            )
        {
            try
            {
                ...
                m_isLocalTransmitBinary = false;
                m_isRemoteTransmitBinary = false;
            ...
            )
        {
            ...
            try
            {
                ...
                if (fSecsDriver.fScdCore.fConfig.enabledEventsOfSecsDeviceData)
                {
            ...
            )
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        procDeviceErrorRaised(FConstants.err_m_16006);
                        return;
                    }
        ...
        #region m_fTelnetClient Methods and Event Handler
            ...
            )
        {
            try
            {
                ...
                this.m_fTelnetClient = new FTelnetClient(fTcpClient);
                // --
                this.m_fTelnetClient.enableService(FTelnetOption.TransmitBinary, true);
                // --
                this.m_fTelnetClient.TelnetStateChanged += new FTelnetStateChangedEventHandler(m_fTelnetClient_TelnetStateChanged);
                this.m_fTelnetClient.TelnetPacketSent += new FTelnetPacketSentEventHandler(m_fTelnetClient_TelnetPacketSent);
                this.m_fTelnetClient.TelnetPacketReceived += new FTelnetPacketReceivedEventHandler(m_fTelnetClient_TelnetPacketReceived);
            }
            catch (Exception ex)
            {
                FDebug.throwException(ex);
        ...
        private void termTelnetClient(
            )
        {
            try
            {
                if (this.m_fTelnetClient != null)
                {
                    this.m_fTelnetClient.Dispose();
                    this.m_fTelnetClient.TelnetStateChanged -= new FTelnetStateChangedEventHandler(m_fTelnetClient_TelnetStateChanged);
                    this.m_fTelnetClient.TelnetPacketSent -= new FTelnetPacketSentEventHandler(m_fTelnetClient_TelnetPacketSent);
                    this.m_fTelnetClient.TelnetPacketReceived -= new FTelnetPacketReceivedEventHandler(m_fTelnetClient_TelnetPacketReceived);
                    this.m_fTelnetClient = null;
                }
            }
            catch (Exception ex)
            {
            ...
            )
        {
            try
            {
                this.m_fTelnetClient.sendWont(FTelnetOption.Status);
                this.m_fTelnetClient.sendDont(FTelnetOption.Status);
                this.m_fTelnetClient.sendWont(FTelnetOption.SuppressGoAhead);
                this.m_fTelnetClient.sendDont(FTelnetOption.SuppressGoAhead);
                this.m_fTelnetClient.sendWont(FTelnetOption.EndOfRecord);
                this.m_fTelnetClient.sendDont(FTelnetOption.EndOfRecord);
                this.m_fTelnetClient.sendDo(FTelnetOption.TransmitBinary);
                this.m_fTelnetClient.sendWill(FTelnetOption.TransmitBinary);
            }
            catch (Exception ex)
            {
                procDeviceErrorRaised(ex);
        ...
        private bool isSelectResponse(
            )
        {
            try
            {
                if (m_isLocalTransmitBinary && m_isRemoteTransmitBinary)
                {
                    return true;
                }
                return false;
        ...
        private void m_fTelnetClient_TelnetStateChanged(
            object sender,
            FTelnetStateChangedEventArgs e
            )
        {
            ...
            try
            {
                ...
                if (fScdCore.fConfig.enabledEventsOfSecsDeviceTelnet)
                {
                    this.fEventPusher.pushSecsDeviceTelnetStateChangedEvent(
                        this.fSecsDevice,
                        FResultCode.Success,
                        string.Empty,
                        e.fOption,
                ...
                if (
                    e.fPosition == FTelnetPosition.Local &&
                    e.fOption == FTelnetOption.TransmitBinary &&
                    e.fOptionState == FTelnetOptionState.On
                    )
                {
                    m_isLocalTransmitBinary = true;
                }
                else if (
                    e.fPosition == FTelnetPosition.Remote &&
                    e.fOption == FTelnetOption.TransmitBinary &&
                    e.fOptionState == FTelnetOptionState.On
                    )
                {
                    m_isRemoteTransmitBinary = true;
                }
        ...
        private void m_fTelnetClient_TelnetPacketSent(
            object sender,
            FTelnetPacketSentEventArgs e
            )
        {
            try
            {
                ...
                if (fScdCore.fConfig.enabledEventsOfSecsDeviceTelnet)
                {
                    this.fEventPusher.pushSecsDeviceTelnetPacketSentEvent(
                        this.fSecsDevice,
                        FResultCode.Success,
                        string.Empty,
                        e.fTelnetPacket
        ...
        private void m_fTelnetClient_TelnetPacketReceived(
            object sender,
            FTelnetPacketReceivedEventArgs e
            )
        {
            try
            {
                ...
                if (fScdCore.fConfig.enabledEventsOfSecsDeviceTelnet)
                {
                    this.fEventPusher.pushSecsDeviceTelnetPacketReceivedEvent(
                        this.fSecsDevice,
                        FResultCode.Success,
                        string.Empty,
                        e.fPacket
            ...
            )
        {
            try
            {
                ...
                else if (e.fState == FTcpClientState.Connected)
                {
                    ...
                    // ***
                    // Telnet 패킷을 이용하여 Select Request를 진행
                    // ***
                    sendSelectRequest();
                }
            ...
            )
        {
            try
            {
                ...
                if (fDeviceState == FDeviceState.Selected)
                {
                    procDataReceived(e.data);
                }
                else
                {
                    m_fTelnetClient.parse(e.data);
                }                
            }
            catch (Exception ex)
            {
           ...
           )
        {
            ...
            try
            {
                ...
                else if (m_fStatusType == FSecs1StatusType.Receive)
                {
                    ...
                    if (m_fTmrT2.elasped(false))
                    {
                        if (fSecsDriver.fScdCore.fConfig.enabledEventsOfSecsDeviceTimeout)
                        {
                            this.fEventPusher.pushSecsDeviceTimeoutRaisedEvent(
                                this.fSecsDevice,
                                FResultCode.Error,
