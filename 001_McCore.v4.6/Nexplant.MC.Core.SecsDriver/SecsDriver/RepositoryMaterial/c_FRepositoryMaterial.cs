...
--  Description     : FAMate Core FaSecsDriver Repository Material Class 
...
namespace Nexplant.MC.Core.FaSecsDriver
{
    public class FRepositoryMaterial : FIObject, IDisposable
    {
        ...
        private HashSet<string> m_secsReplyKeys = null;
            ...
            )
        {
            m_fScdCore = fScdCore;
            m_fXmlNode = fXmlNode;
            m_secsReplyKeys = new HashSet<string>();
        ...
        public FColumnCollection fChildColumnCollection
        {
            get
            {
                try
                {
                    return new FColumnCollection(m_fScdCore, this.fXmlNode.selectNodes(FXmlTagCOLL.E_Column));
                }
                catch (Exception ex)
                {
                    FDebug.throwException(ex);
        ...
        private string makeSecsReplyKey(
            UInt64 sdvUniqueId,
            UInt64 ssnUniqueId,
            UInt32 systemBytes
            )
        {
            try
            {
                return string.Format(ReplyKeyFormat, sdvUniqueId, ssnUniqueId, systemBytes);
            }
            catch (Exception ex)
            {
                FDebug.throwException(ex);
            ...
            )
        {
            ...
            try
            {
                key = makeSecsReplyKey(sdvUniqueId, ssnUniqueId, systemBytes);
                // --
                if (!m_secsReplyKeys.Contains(key))
                {
                    m_secsReplyKeys.Add(key);
                }
            }
            catch (Exception ex)
            {
        ...
        internal bool containsSecsReplyKey(
            UInt64 sdvUniqueId, 
            UInt64 ssnUniqueId, 
            UInt32 systemBytes
            )
        {
            try
            {
                return m_secsReplyKeys.Contains(makeSecsReplyKey(sdvUniqueId, ssnUniqueId, systemBytes));
            }
            catch (Exception ex)
            {
                FDebug.throwException(ex);
            ...
            )
        {
            ...
            try
            {
                ...
                return new FColumn(m_fScdCore, fXmlNode);
            }
            catch (Exception ex)
            {
                FDebug.throwException(ex);
