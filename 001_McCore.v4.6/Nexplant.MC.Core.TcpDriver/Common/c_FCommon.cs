...
--  Description     : FAMate Core FaTcpDriver Common Function Class 
...
namespace Nexplant.MC.Core.FaTcpDriver
{
    internal static class FCommon
    {
            ...
            )
        {
            ...
            try
            {
                if (fXmlNode.name == FXmlTagTMS.E_TcpMessages)
                {
                    if (fXmlNode.get_attrVal(FXmlTagTMS.A_Command, FXmlTagTMS.D_Command).ToLower().Contains(searchWord))
                    {
                        return true;
                    }
                }
                else if (fXmlNode.name == FXmlTagTMG.E_TcpMessage)
                {
                    if (fXmlNode.get_attrVal(FXmlTagTMG.A_Command, FXmlTagTMG.D_Command).ToLower().Contains(searchWord))
                    {
                        return true;
                    }
                }
                else if (fXmlNode.name == FXmlTagTIT.E_TcpItem)
                {
                    encodingValue = FValueConverter.toDataConversionedEncodingValue(
                        FEnumConverter.toFormat(fXmlNode.get_attrVal(FXmlTagTIT.A_Format, FXmlTagTIT.D_Format)),
                        fXmlNode.get_attrVal(FXmlTagTIT.A_Value, FXmlTagTIT.D_Value),
                        fXmlNode.get_attrVal(FXmlTagTIT.A_Transformer, FXmlTagTIT.D_Transformer),
                        fXmlNode.get_attrVal(FXmlTagTIT.A_DataConversionSetExpression, FXmlTagTIT.D_DataConversionSetExpression)
                        );
