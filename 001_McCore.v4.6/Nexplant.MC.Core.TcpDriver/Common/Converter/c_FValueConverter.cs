...
--  Description     : FAMate Core FaTcpDriver Value Converter Class 
                ...
                foreach (string valueFormula in conversionExpList)
                {
                    ...
                    if (fDce.fComparisonMode == FComparisonMode.Value)
                    {
                        ...
                        else
                        {
                            ...
                            if (!FTcpDriverCommon.validateFormatRange(fFormat, fDce.stringValue))
                            {
                                continue;
                            }

                            // Covversion Value 가 실제 Item 의 Format 으로 표현할수 없으면 Pass.
                            if (!FTcpDriverCommon.validateFormatRange(fFormat, conversionValue))
                            {
                                continue;
                            }
