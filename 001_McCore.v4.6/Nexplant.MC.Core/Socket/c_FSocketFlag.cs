...
--
--  Program Id      : c_FSocketFlag.cs
--  Creator         : spike.lee
--  Create Date     : 2011.08.26
--  Description     : FAMate Core FaCommon Socket Flag Class
--  History         : Created by spike.lee at 2011.08.26
----------------------------------------------------------------------------------------------------------*/
using System;
...
{
    public class FSocketFlag : IDisposable
    {
        ...
        public bool doAcceptCompleted
        {
            get
            {
                try
                {
                    return m_acceptCount == 0 ? true : false;
                }
                catch (Exception ex)
                {
        ...
        public bool doConnectCompleted
        {
            get
            {
                try
                {
                    return m_connectCount == 0 ? true : false;
                }
                catch (Exception ex)
                {
                    FDebug.throwException(ex);
        ...
        public void waitAcceptCompleted(
            )
        {
            try
            {
                while (!this.doAcceptCompleted)
                {
        ...
        public void waitConnectCompleted(
            )
        {
            try
            {
                while (!this.doConnectCompleted)
                {
                    System.Threading.Thread.Sleep(1);
                }
            }
            ...
            )
        {
            try
            {
                while (!this.doAcceptCompleted || !this.doConnectCompleted || !this.doSendCompleted || !this.doReceiveCompleted)
                {
                    System.Threading.Thread.Sleep(1);
                }
            }
