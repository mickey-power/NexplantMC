...
--  Description     : FAMate Core FaOpcDriver PLC Expression Value Transformer Class 
...
{
    public class FOpcExpressionValueTransformer : FValueTransformerBase, FIValueTransformer
    {
        ...
        internal FOpcExpressionValueTransformer(
            FOpcExpression fOep
            ) 
            : base()
        {
        ...
        ~FOpcExpressionValueTransformer(
           )
        {
            myDispose(false);
        }
        ...
        public FIValueFormulaCollection fValueFormulaCollection
        {
            get
            {
                try
                {
                    return new FOpcExpressionValueFormulaCollection(m_fOep.fXmlNode);
                }
                catch (Exception ex)
                {
                    FDebug.throwException(ex);
            ...
            )
        {
            ...
            try
            {
                val = m_fOep.fXmlNode.get_attrVal(FXmlTagOEP.A_Transformer, FXmlTagOEP.D_Transformer);
                if (val == string.Empty)
                {
            ...
            )
        {
            try
            {
                ...
                m_fOep.fXmlNode.set_attrVal(
                    FXmlTagOEP.A_Transformer,
                    FXmlTagOEP.D_Transformer,
