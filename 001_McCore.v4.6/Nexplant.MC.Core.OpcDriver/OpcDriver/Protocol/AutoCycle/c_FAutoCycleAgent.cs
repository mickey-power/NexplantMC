...
--  Description     : FAMate Core FaOpcDriver Auto Cycle Agent Class 
...
namespace Nexplant.MC.Core.FaOpcDriver
{
    internal class FAutoCycleAgent : IDisposable
    {
        ...
        public void collectOpcAutoCycleData(
            ...
            )
        {
            const string Query =
                ...
                "[" +
                "@" + FXmlTagOTN.A_UsedAutoCycle + "='{0}' and " +
            ...
            try
            {
                fXmlNodeListOtn = m_fOpcDriver.fXmlNode.selectNodes(string.Format(Query, FBoolean.True, fOpcDevice.uniqueIdToString));
                if (fXmlNodeListOtn.count == 0)
                {
                ...
                // --
                foreach (FXmlNode fXmlNodeOtn in fXmlNodeListOtn)
                {
                    uniqueId = fXmlNodeOtn.get_attrVal(FXmlTagOTN.A_UniqueId, FXmlTagOTN.D_UniqueId);
                    period = int.Parse(fXmlNodeOtn.get_attrVal(FXmlTagOTN.A_AutoCyclePeriod, FXmlTagOTN.D_AutoCyclePeriod));
                    fAutoCycleAction = FEnumConverter.toAutoCycleAction(fXmlNodeOtn.get_attrVal(FXmlTagOTN.A_AutoCycleAction, FXmlTagOTN.D_AutoCycleAction));
                    ...
                    if (m_autoCycleKeys.Contains(uniqueId))
                    {
                        ...
                        else
                        {
                            ...
                            // --
                            newAutoCycleKeys.Add(uniqueId);
                            newAutoCycleList.Add(uniqueId, new FAutoCycleData(uniqueId, period, fAutoCycleAction, fXmlNodeOtn));
                        }
                    }
                    else
                    {
            ...
            )
        {
            ...
            try
            {
                fXmlNodeListOtn = m_fOpcDriver.fXmlNode.selectNodes(string.Format(Query, FBoolean.True, fOpcSession.uniqueIdToString));
                if (fXmlNodeListOtn.count == 0)
                {
                    if (m_autoCycleKeys.Count > 0)
                    {
            ...
            )
        {
            ...
            try
            {
                fXmlNodeListHtn = m_fOpcDriver.fXmlNode.selectNodes(string.Format(Query, FBoolean.True, fHostDevice.uniqueIdToString));
