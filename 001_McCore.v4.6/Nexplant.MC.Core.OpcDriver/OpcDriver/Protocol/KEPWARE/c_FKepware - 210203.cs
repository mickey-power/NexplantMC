...
namespace Nexplant.MC.Core.FaOpcDriver
{
    internal class FKepware : IDisposable
    {
        ...
        private bool m_isFirstWriteError = true;
        //--
            ...
            )
        {
            ...
            {
                if (
                    this.fOpcDriver.fOpcRunMode == FOpcRunMode.WorkspaceManager ||
                    m_isFirstWriteError
                    )
                {
                    procDeviceErrorRaised(ex, FResultCode.Error);
                    m_isFirstWriteError = false;
                }
            ...
            )
        {
            ...
            try
            {
                    ...
                    retCode == ReturnCode.UNSUPPORTEDUPDATERATE)
                {
                    ...
                    dla = new FDebugLogArgs(FDebugLogCategory.Information, "Subscribe", this.GetType(), "procSubscribereloadBySession");
                    dla.additionInfo = string.Format("Session={0}, ServerHandle={1}", fOpcSession.name, serverHandle);
                    FDebug.writeLog(dla);
                }
                else
                {
                    ...
                    if (this.fOpcDriver.fOpcRunMode == FOpcRunMode.WorkspaceManager)
                    {
                        foreach (ItemIdentifier opcItem in opcIdList)
                        {
                            ...
                            {
                                procDeviceErrorRaised(new Exception(string.Format(FConstants.err_m_0044, fNewSession.name, opcItem.ItemName.Substring(7).ToString())), FResultCode.Error);
                            }
                        }
                    }
                    else
            ...
            catch (OPCException oex)
            {
                ...
                {
                    dla = new FDebugLogArgs(FDebugLogCategory.FException, "SubscribeError", this.GetType(), "procSubscribereloadBySession");
                    dla.additionInfo = string.Format("Session={0}, ServerHandle={1}", fOpcSession.name, serverHandle);
                    FDebug.writeLog(dla);
                }
            ...
            try
            {
                ...
                foreach (ulong key in m_fSubscriberPlcState.fSessionList.Keys)
                {
                        ...
                        retCode == ReturnCode.UNSUPPORTEDUPDATERATE)
                    {
                        ...
                        // --
                        dla = new FDebugLogArgs(FDebugLogCategory.Information, "Subscribe", this.GetType(), "procPlcConnectTagReload");
                        dla.additionInfo = "Plc connect subscribe success";
                        FDebug.writeLog(dla);
                    }
                    else
                    {
