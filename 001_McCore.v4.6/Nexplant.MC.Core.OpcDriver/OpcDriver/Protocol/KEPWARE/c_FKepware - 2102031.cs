...
namespace Nexplant.MC.Core.FaOpcDriver
{
    internal class FKepware : IDisposable
    {
        ...
        // --        
        private FKepwareSubscriber m_fTagSubscriber = null;
        private FKepwareSubscriber m_fSubscriberPlcState = null;
        // --
        ...
        private ItemValueCallback[] m_subscribeItem = null;
            ...
            )
        {
            try
            {
                ...
                m_fTagSubscriber = new FKepwareSubscriber();
                // --
                // Add by Jeff.Kim 2015.10.06
                // System.ErrorTag 분리
                m_fSubscriberPlcState = new FKepwareSubscriber();
                // --
            ...
            )
        {
            ...
            try
            {
                ...
                if (m_fTagSubscriber != null)
                {
                    m_fTagSubscriber.Dispose();
                    m_fTagSubscriber = null;
                }
                // --
                // Add by Jeff.Kim 2015.10.06
                // System.ErrorTag 분리
                if (m_fSubscriberPlcState != null)
                {
                    m_fSubscriberPlcState.Dispose();
                    m_fSubscriberPlcState = null;
                }
            ...
            )
        {
            ...
            try
            {
                ...
                {
                    m_opcServer.Browse(
                        string.Format(FConstants.BrowerDaItemNameFormat, fOpcSession.channel),
                        "",
                        ref cp,
                        0,
                        BrowseFilter.ITEM,
            ...
            )
        {
            try
            {
                m_fTagSubscriber.init();
                m_fSubscriberPlcState.init();
            ...
            {
                procDeviceErrorRaised(ex, FResultCode.Error);
            }
            finally
            {
        ...
        private void procPlcDisconnect(
            FOpcSession fOpcSession,
            ...
            )
        {
            try
            {
                ...
                procSubscribeReset(fKepwareSession);
            }
            catch (Exception ex)
            {
                procDeviceErrorRaised(ex, FResultCode.Error);
        ...
        private void procSubscribeDataChange(
            ...
            )
        {
            ...
            try
            {
                if (!m_fTagSubscriber.fSessionList.TryGetValue((UInt64)clientSubscription, out fKepSession))
                {
                    return;
                }
                ...
                if (!allQualitiesGood)
                {
                    ...
                    exMsg = string.Format(FConstants.err_m_0039, "Subscribe error", info);
                    FDebug.throwFException(exMsg);
                }

                // --
                // Add by Jeff.Kim 2015.10.09
                // Tag를 등록하고 처음으로 Changed 이벤트가 발생할경우 Tag별 상태 확인하여 Error 발생
                if (!fKepSession.firstDataChanged)
                {
                    fKepSession.firstDataChanged = true;
                    foreach (ItemValueCallback v in itemValues)
                    {
                        if (!v.ResultID.Succeeded)
                        {
                            info = "OpcSession=" + fOpcSession.name + ", ItemResultID=" + v.ResultID.ToString();
                            exMsg = string.Format(FConstants.err_m_0039, "Subscribe error", info);
                            FDebug.throwFException(exMsg);
                        }
                    }
                ...
                if (allQualitiesGood)
                {
                    if (this.fOpcDevice.fState != FDeviceState.Selected)
                    {
                        m_subscribeItem = itemValues;
                    }
                    else 
                    {
                        procSubscribeItem(fKepSession, itemValues);
                    }
            ...
            catch (Exception ex)
            {
                ...
                {
                    procDeviceErrorRaised(ex, FResultCode.Error);
                    procPlcDisconnect(fOpcSession, fKepSession);
                }
                //else
                //{
                //    if (fKepSession != null)
                //    {
                //        procSubscribeReset(fKepSession);
                //    }
                //}
        ...
        private void procSubscribePlcStateDataChange(
            int clientSubscription,
            bool allQualitiesGood,
            bool noErrors,
            ItemValueCallback[] itemValues
            )
        {
            ...
            {
                if (
                    !m_fSubscriberPlcState.enabled ||
                    !m_fSubscriberPlcState.fSessionList.TryGetValue((UInt64)clientSubscription, out fKepSession)
                    )
                {
                    return;
                }
                ...
                if (!allQualitiesGood)
                {
                    info = "PlcStateSubscribe" + ", allQualitiesGood=" + allQualitiesGood.ToString();
                    exMsg = string.Format(FConstants.err_m_0039, "Subscribe error", info);
                    fTargetOsn = this.fOpcDevice.selectSingleOpcSessionByUniqueId(fKepSession.osnUniqueId);
                    FDebug.throwFException(exMsg);
                }
                ...
                if (!fKepSession.firstDataChanged)
                {
                    ...
                    foreach (ItemValueCallback v in itemValues)
                    {
                        if (!v.ResultID.Succeeded)
                        {
                            info = "PlcStateSubscribe" + ", ItemResultID=" + v.ResultID.ToString();
                            exMsg = string.Format(FConstants.err_m_0039, "Subscribe error", info);
                            FDebug.throwFException(exMsg);
                        }
                    }
                ...
                foreach (ItemValueCallback v in itemValues)
                {
                    fTargetOsn = this.fOpcDevice.selectSingleOpcSessionByUniqueId((UInt64)v.ClientHandle);
                    if (fTargetOsn == null)
                    {
                        // ***
                        // HEARTBEAT Tag
                        ...
                        if (v.Value == null)
                        {
                            info = "PlcStateSubscribe" + ", ItemResultID=(HEARTBEAT)Read Error";
                            exMsg = string.Format(FConstants.err_m_0039, "Subscribe error", info);
                            FDebug.throwFException(exMsg);
                        }
                    ...
                    if ((v.Value is bool) && (bool)v.Value)
                    {
                        //--
                        foreach (FOpcSession fOsn in this.fOpcDevice.fChildOpcSessionCollection)
                        {
                            if (fTargetOsn.channel == fOsn.channel)
                            {
                                info = "PlcStateSubscribe" + ", OpcSession=" + fOsn.name;
                                exMsg = string.Format(FConstants.err_m_0039, "Subscribe error", info);
                                FDebug.throwFException(exMsg);
                            }
                        }
            ...
            catch (Exception ex)
            {
                ...
                {
                    procDeviceErrorRaised(ex, FResultCode.Error);
                    procPlcDisconnect(fTargetOsn, fKepSession);
                }
                else
                {
                    //if (m_fSubscriberPlcState.enabled)
                    //{
                    //    procPlcConnectTagReset();
                    //}
                }
            ...
            )
        {
            try
            {
                ...
                {
                    this.fEventPusher.pushOpcDeviceErrorRaisedEvent(this.fOpcDevice, rstCode, inEx.Message);
                }
            }
            catch (Exception ex)
            {
            ...
            )
        {
            ...
            try
            {
                ...
                foreach (FOpcEventItem fOei in fOmt.fChildEventItemListCollection[0].fChildOpcEventItemCollection)
                {
                    ...
                    if (m_fProtocol == FProtocol.OPCDA)
                    {
                        tagId.ItemName = string.Format(FConstants.DaItemNameFormat, fOsn.channel, fOei.itemName);
                    }
                    else
                    {
                        tagId.ItemName = string.Format(m_itemNameFormat, m_defaultNamespace, fOsn.channel, fOei.itemName);
                    }
                ...
                foreach (FOpcItem fOit in fOmt.fChildOpcItemListCollection[0].fChildOpcItemCollection)
                {
                    ...
                    if (m_fProtocol == FProtocol.OPCDA)
                    {
                        tagId.ItemName = string.Format(FConstants.DaItemNameFormat, fOsn.channel, fOit.itemName);
                    }
                    else
                    {
                        tagId.ItemName = string.Format(m_itemNameFormat, m_defaultNamespace, fOsn.channel, fOit.itemName);
                    }
        ...
        public void procSubscribeReload(
            )
        {
            try
            {
        ...
        private void procSubscribeReloadBySesssion(
            FOpcSession fOpcSession
            )
        {
            const string xPath =
            ...
            try
            {
                ...
                if (m_fTagSubscriber.fSessionList.ContainsKey(fOpcSession.uniqueId))
                {
                    fOldSession = m_fTagSubscriber.fSessionList[fOpcSession.uniqueId];
                    if (fOldSession.registerSucceed && !fOldSession.completed)
                    {
                        // --

                        if (fOldSession.serverHandle >= 0)
                        {
                            // 기존 등록이 완료 되었는데, Reload 시점에 DataChanged Event 를 받지 못했다면, 서버로 
                            // 취소 보낸후 다시 등록 한다. 
                            try
                            {
                                m_opcServer.SubscriptionCancel(fOldSession.serverHandle);
                            }
                            catch { }
                        }
                        ...
                        m_fTagSubscriber.fSessionList.Remove(fOpcSession.uniqueId);
                ...
                foreach (FXmlNode x in fNodeItemList)
                {
                    ...
                    if (m_fProtocol == FProtocol.OPCDA)
                    {
                        itemName = string.Format(FConstants.DaItemNameFormat, fOpcSession.channel, x.get_attrVal(FXmlTagOEI.A_ItemName, FXmlTagOEI.D_ItemName));
                    }
                    else
                    {
                        itemName = string.Format(m_itemNameFormat, m_defaultNamespace, fOpcSession.channel, x.get_attrVal(FXmlTagOEI.A_ItemName, FXmlTagOEI.D_ItemName));
                    }
                    ...
                    retCode == ReturnCode.UNSUPPORTEDUPDATERATE)
                {
                    ...
                    // --
                    m_fTagSubscriber.fSessionList.Add(fNewSession.osnUniqueId, fNewSession);

                    // --

#if DEBUG
                    FDebugLogArgs fDebugLogArg = null;
                    fDebugLogArg = new FDebugLogArgs(FDebugLogCategory.Information, "Subscribe", this.GetType(), "procSubscribereloadBySession");
                    fDebugLogArg.additionInfo = string.Format("Session={0}, ServerHandle={1}", fOpcSession.name, serverHandle);
                    FDebug.writeLog(fDebugLogArg);
#endif
                }
                else
                {
                    ...
                    if (this.fOpcDriver.fOpcRunMode == FOpcRunMode.WorkspaceManager)
                    {
                        foreach (ItemIdentifier opcItem in opcIdList)
                        {
                            ...
                            {
                                FDebug.throwFException(string.Format(FConstants.err_m_0044, fNewSession.name, opcItem.ItemName.Substring(7).ToString()));
                            }
                        }
                    }
                    else
                    {
                        FDebug.throwFException(string.Format(FConstants.err_m_0043, fNewSession.name, "Data", "ReturnCode=" + retCode.ToString()));
                    }
                }
            }
            ...
            {
                // ***
                //  Add at 2019.07.03 by Kt.Kim
                //  OPC Exception 관련 사항 추가
                // ***

#if DEBUG
                if (oex.Message.Contains("Unspecified"))
                {
                    FDebugLogArgs fDebugLogArgs = null;
                    fDebugLogArgs = new FDebugLogArgs(FDebugLogCategory.FException, "SubscribeError", this.GetType(), "procSubscribereloadBySession");
                    fDebugLogArgs.additionInfo = string.Format("Session={0}, ServerHandle={1}", fOpcSession.name, serverHandle);
                    FDebug.writeLog(fDebugLogArgs);
                }
#endif 
                FDebug.throwException(oex);
            }
            catch (Exception ex)
            {
                if (this.fOpcDevice.fState == FDeviceState.Selected)
                {
                    procDeviceErrorRaised(ex, FResultCode.Error);
                    procPlcDisconnect(fOpcSession, fNewSession);
                }
                //else
                //{
                //    if (fNewSession != null)
                //    {
                //        procSubscribeReset(fNewSession);
                //    }
                //}
            }
            finally
        ...
        private void procPlcConnectTagReload(
            )
        {
            // --
            ...
            try
            {
                if (m_fSubscriberPlcState.succeed)
                {
                    return;
                }
                ...
                foreach (FOpcSession fOsn in this.fOpcDevice.fChildOpcSessionCollection)
                {
                    ...
                    if (!duplicatedChannels.Contains(fOsn.channel))
                    {
                        ...
                        if (m_fProtocol == FProtocol.OPCDA)
                        {
                            itemName = string.Format(FConstants.DaItemNameFormat, fOsn.channel, FConstants.SystemErrorTagName);
                        }
                        else
                        {
                            itemName = string.Format(m_itemNameFormat, m_defaultNamespace, fOsn.channel, FConstants.SystemErrorTagName);
                        }
                        ...
                        if (m_fProtocol == FProtocol.OPCDA)
                        {
                            itemName = string.Format(FConstants.DaItemNameFormat, fOsn.channel, FConstants.HeartbeatTagName);
                        }
                        else
                        {
                            itemName = string.Format(m_itemNameFormat, m_defaultNamespace, fOsn.channel, FConstants.HeartbeatTagName);
                        }
                ...
                if (m_fSubscriberPlcState.enabled)
                {
                    foreach (FKepwareSession s in m_fSubscriberPlcState.fSessionList.Values)
                    {
                ...
                m_fSubscriberPlcState.fSessionList.Clear();
                m_fSubscriberPlcState.fSessionList = fNewSessionList;
                // --
                if (m_fSubscriberPlcState.fSessionList.Count == 0)
                {
                    m_fSubscriberPlcState.enabled = false;
                    return;
                }
                ...
                // 신규 Subscribe 등록
                // ***                
                m_fSubscriberPlcState.enabled = true;
                // --
                foreach (ulong key in m_fSubscriberPlcState.fSessionList.Keys)
                {
                    fNewSession = m_fSubscriberPlcState.fSessionList[key];
...
#if DEBUG
                        FDebugLogArgs fDebugLogArgs = null;
                        fDebugLogArgs = new FDebugLogArgs(FDebugLogCategory.Information, "Subscribe", this.GetType(), "procPlcConnectTagReload");
                        fDebugLogArgs.additionInfo = "Plc connect subscribe success";
                        FDebug.writeLog(fDebugLogArgs);
#endif
                    }
                    else
                    {
                        // ***
                        // Kepware connection 상태 확인을 위한 HEARTBEAT가 등록되지 않은 상태
                        // Critical Error Event를 발생시킨다.
                        // ***
                        if (m_isFirstConnect && (m_fDeviceState == FDeviceState.Selected) ||
                            !m_isFirstConnect)
                        {
                            this.fEventPusher.pushOpcDeviceErrorRaisedEvent(
                            this.fOpcDevice,
                            FResultCode.Warninig,
                            "Plc connect tag (HEARTBEAT) is not registered."
                            );
                        }
        ...
        private void procPlcConnectTagReset(
            )
        {
            try
            {
        ...
        private void procSubscribeReset(
            FKepwareSession fKepwareSession
            )
        {
            try
            {
                ...
                // 이전 Subscribe 중지
                // ***
                if (m_fTagSubscriber.fSessionList.ContainsKey(fKepwareSession.osnUniqueId))
                {
                    if (fKepwareSession.registerSucceed)
                    {
                        m_opcServer.SubscriptionCancel(fKepwareSession.serverHandle);
                    }
                    m_fTagSubscriber.fSessionList.Remove(fKepwareSession.osnUniqueId);
                }
                else
                {
                    procPlcConnectTagReset();
        ...
        private void procSubscribeItem(
            ...
            )
        {
            ...
            try
            {
                foreach (ItemValueCallback c in itemValues)
                {
                    ...
                    // ***
                    if (fKepSession.osnUniqueId == oeiUniqueId)
                    {
                        continue;
                    }
                    ...
                    if (!fKepSession.fItemList.ContainsKey(oeiUniqueId))
                    {
                        continue;
                    }
                    ...
                    if (fOsn == null || oldUniqueId != osnUniqueId)
                    {
            ...
            )
        {
            try
            {
                foreach (FDelayMessage m in fDelayMessageList)
                {
                    ...
                    {
                        procDeviceErrorRaised(inEx, FResultCode.Error);
                    }
                    finally
                    {
           ...
           )
        {
            ...
            try
            {
                ...
                if (m_fDeviceState == FDeviceState.Closed && !m_isFirstConnect)
                {
                ...
                else if (m_fDeviceState == FDeviceState.Selected)
                {
                    if (m_fProtocol == FProtocol.KEPWARE)
                    {
                        if (!m_fSubscriberPlcState.succeed)
                ...
                for (int i = 0; i < this.fOpcDevice.fChildOpcSessionCollection.count; i++)
                {
                    ...
                    if (
                        m_fTagSubscriber.fSessionList.ContainsKey(fOpcSession.uniqueId) &&
                        m_fTagSubscriber.fSessionList[fOpcSession.uniqueId].registerSucceed &&
                        m_fTagSubscriber.fSessionList[fOpcSession.uniqueId].completed &&
                        m_fDeviceState == FDeviceState.Connected
                        )
                    {
                        if (
                            (m_isFirstConnect && m_fSubscriberPlcState.completed) ||
                            (!m_isFirstConnect && m_fSubscriberPlcState.succeed && m_fSubscriberPlcState.completed)
                            )
                        {
                            procStateSelected(fOpcSession);
                            // --
                            if (m_subscribeItem != null)
                            {
                                procSubscribeItem(m_fTagSubscriber.fSessionList[fOpcSession.uniqueId], m_subscribeItem);
                                m_subscribeItem = null;
                            }
                        }
                    }
                    ...
                    else if (!m_fTagSubscriber.fSessionList.ContainsKey(fOpcSession.uniqueId) ||
                        (m_fTagSubscriber.fSessionList[fOpcSession.uniqueId].registerSucceed && !m_fTagSubscriber.fSessionList[fOpcSession.uniqueId].completed && m_fSubscriberPlcState.completed))
                    {
                        if (m_fTmrTagReloadList[fOpcSession.uniqueId].elasped(false))
                        {
                            procSubscribeReloadBySesssion(fOpcSession);
                    ...
                    if (fOcdCore.fConfig.enabledEventsOfScenario)
                    {
                        if (m_fTmrAutoCycleList.ContainsKey(fOpcSession.uniqueId))
                        {
                            if (m_fTmrAutoCycleList[fOpcSession.uniqueId].elasped(true))
                            {
                                isSleep = false;
                    ...
                    {
                        if (m_fTmrAutoTraceList[fOpcSession.uniqueId].elasped(true))
                        {
                            isSleep = false;
            ...
            try
            {
                ...
                while (m_fQueueSubscribeData.count > 0)
                {
                    try
                    {
                        ...
                        {
                            procSubscribePlcStateDataChange(
