...
--  Description     : FAMate Core FaOpcDriver Value Converter Class 
                ...
                foreach (string valueFormula in conversionExpList)
                {
                    ...
                    if (fDce.fComparisonMode == FComparisonMode.Value)
                    {
                        ...
                        else
                        {
                            ...
                            if (!FOpcDriverCommon.validateFormatRange(fFormat, fDce.stringValue))
                            {
                                continue;
                            }

                            // Covversion Value 가 실제 Item 의 Format 으로 표현할수 없으면 Pass.
                            if (!FOpcDriverCommon.validateFormatRange(fFormat, conversionValue))
                            {
                                continue;
                            }
