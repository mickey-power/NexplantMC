...
--  Description     : FAMate Core FaOpcDriver Common Function Class 
...
namespace Nexplant.MC.Core.FaOpcDriver
{
    internal static class FCommon
    {
            ...
            )
        {
            ...
            try
            {
                if (fXmlNode.name == FXmlTagOEI.E_OpcEventItem)
                {
                    encodingValue = FValueConverter.toTransformedEncodingValue(
                        FEnumConverter.toFormat(fXmlNode.get_attrVal(FXmlTagOEI.A_Format, FXmlTagOEI.D_Format)),
                        fXmlNode.get_attrVal(FXmlTagOEI.A_Value, FXmlTagOEI.D_Value),
                        fXmlNode.get_attrVal(FXmlTagOEI.A_Transformer, FXmlTagOEI.D_Transformer)
                        );                    
                    ...
                    // --
                    if (fXmlNode.get_attrVal(FXmlTagOEI.A_ItemName, FXmlTagOEI.D_ItemName).ToLower().Contains(searchWord))
                    {
                        return true;
                    }
                }
                else if (fXmlNode.name == FXmlTagOIT.E_OpcItem)
                {
                    encodingValue = FValueConverter.toDataConversionedEncodingValue(
                        FEnumConverter.toFormat(fXmlNode.get_attrVal(FXmlTagOIT.A_Format, FXmlTagOIT.D_Format)),
                        fXmlNode.get_attrVal(FXmlTagOIT.A_Value, FXmlTagOIT.D_Value),
                        fXmlNode.get_attrVal(FXmlTagOIT.A_Transformer, FXmlTagOIT.D_Transformer),
                        fXmlNode.get_attrVal(FXmlTagOIT.A_DataConversionSetExpression, FXmlTagOIT.D_DataConversionSetExpression)
                        );
                    if (encodingValue.ToLower().Contains(searchWord))
                    {
                        return true;
