...
--
--  Program Id      : f_FSmlViewer.cs
--  Creator         : kitae
--  Create Date     : 2011.06.01
--  Description     : FAMate SECS Modeler SML Viewer Form Class
--  History         : Created by kitae at 2011.06.01
                      Modify by spike.lee at 2012.11.14
...
{
    public partial class FSmlViewer : Nexplant.MC.Core.FaUIs.FBaseTabChildForm
    {
            ...
            )
        {
            try
            {
                ...
                mnuMenu.Tools[FMenuKey.menuSmvRefresh].SharedProps.Enabled = !m_isNewFile;
                // --
                mnuMenu.Tools[FMenuKey.MenuSmvFind].SharedProps.Enabled = true;
                // --
                mnuMenu.Tools[FMenuKey.MenuSmvOpen].SharedProps.Enabled = true;                
                // --
                mnuMenu.Tools[FMenuKey.MenuSmvSave].SharedProps.Enabled = m_isNewFile | m_isModifiedFile;
                mnuMenu.Tools[FMenuKey.MenuSmvSaveAs].SharedProps.Enabled = true;
                // --
                mnuMenu.Tools[FMenuKey.MenuSmvClear].SharedProps.Enabled = !m_isCleared;                                
        ...
        public void appendSml(
            string smlText
            )
        {           
            try
            {
                txtSml.AppendText(smlText);
                txtSml.Select(txtSml.TextLength, 0);
                txtSml.ScrollToCaret();
                txtSml.Focus();
            ...
            )
        {
            ...
            try
            {
                ...
                // --
                m_fileName = fileName;
                m_fSsmCore.fOption.smlViewerRecentOpenPath = Path.GetDirectoryName(m_fileName);
            ...
            )
        {
            ...
            try
            {
                ...
                // --
                m_fileName = fileName;
                m_fSsmCore.fOption.smlViewerRecentSavePath = Path.GetDirectoryName(m_fileName);
            ...
            )
        {
            ...
            try
            {
                ...
                dialog.InitialDirectory = m_fSsmCore.fOption.smlViewerRecentOpenPath;
                dialog.Title = fUIWizard.searchCaption("Open SML Log File");
                dialog.Filter = "SML Log Files (*.sml) | *.sml";
                dialog.DefaultExt = "sml";
            ...
            )
        {
            ...
            try
            {
                ...
                dialog.Title = fUIWizard.searchCaption("Save SML Log File");
                dialog.Filter = "SML Log File (*.sml) | *.sml";
                dialog.DefaultExt = "sml";
                // --
                if (m_isNewFile)
                {
                    dialog.InitialDirectory = m_fSsmCore.fOption.smlViewerRecentSavePath;
            ...
            )
        {
            ...
            try
            {
                ...
                // --
                txtSml.Appearance.FontData.Name = (new System.Drawing.FontFamily(preFontName)).Name;
            ...
            )
        {
            try
            {
                txtSml.Appearance.FontData.SizeInPoints = float.Parse(mnbFontSize.Value.ToString());
        ...
        #region FSmlViewer Form Event Handler

        private void FSmlViewer_Load(
            object sender,
            EventArgs e
            )
        {
            try
            {
                ...
                txtSml.Appearance.FontData.Name = ((FontListTool)mnuMenu.Tools[FMenuKey.MenuFontName]).Value.ToString();
                txtSml.Appearance.FontData.SizeInPoints = float.Parse(mnbFontSize.Value.ToString());

                // --

                m_fileName = "SML" + m_fFileIdPointer.uniqueId.ToString() + ".sml";
                // --
                m_isNewFile = true;
                m_isModifiedFile = false;
                m_isCleared = true;
        ...
        private void FSmlViewer_FormCloseConfirm(
            object sender, 
        ...
        private void FSmlViewer_FormClosing(
            object sender, 
        ...
        #region txtSml Control Event Handler

        private void txtSml_KeyDown(
            object sender,
            KeyEventArgs e
            )
        {
            ...
            )
        {
            try
            {
                ...
                if (e.Tool.Key == FMenuKey.menuSmvRefresh)
                {
                    procMenuRefresh();
                }
                else if (e.Tool.Key == FMenuKey.MenuSmvFind)
                {
                    txtSml.showSearcher();
                }
                else if (e.Tool.Key == FMenuKey.MenuSmvOpen)
                {
                    procMenuOpen();
                }
                else if (e.Tool.Key == FMenuKey.MenuSmvSave)
                {
                    procMenuSave();
                }
                else if (e.Tool.Key == FMenuKey.MenuSmvSaveAs)
                {
                    procMenuSaveAs();
                }
                else if (e.Tool.Key == FMenuKey.MenuSmvClear)
                {
                    procMenuClear();
                }                
            }
            ...
            )
        {
            try
            {
                ...
                if (e.Tool.Key == FMenuKey.MenuFontName)
                {
                    ...
                    {
                        ((FontListTool)mnuMenu.Tools[FMenuKey.MenuFontName]).Value = m_fSsmCore.fOption.commonFontName;
                        txtSml.Appearance.FontData.Name = m_fSsmCore.fOption.commonFontName;
                    }
                }
            }
            catch (Exception ex)
