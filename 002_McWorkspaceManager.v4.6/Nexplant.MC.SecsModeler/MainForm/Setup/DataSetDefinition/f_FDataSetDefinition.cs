...
--  Description     : FAMate DataSet Definition Form Class 
    ...
    public partial class FDataSetDefinition : Nexplant.MC.Core.FaUIs.FBaseTabChildForm
    {
            ...
            try
            {
                ...
                foreach (ToolBase t in mnuMenu.Tools)
                {
                        ...
                        )
                    {
                        ...
                        t.Key == FMenuKey.MenuDsdImportText || 
                        t.Key == FMenuKey.MenuDsdValidation
                        )
                    {
                ...
                {
                    mnuMenu.Tools[FMenuKey.MenuDsdAppendDataSetList].SharedProps.Visible = ((FSecsDriver)fObject).canAppendChildDataSetList;
                    // --
                    mnuMenu.Tools[FMenuKey.MenuDsdPasteChild].SharedProps.Enabled = ((FSecsDriver)fObject).canPasteChildDataSetList;
                    // --
                    mnuMenu.Tools[FMenuKey.MenuDsdValidation].SharedProps.Enabled = true;
                }
                ...
                else if (fObject.fObjectType == FObjectType.Data)
                {
                    ...
                    mnuMenu.Tools[FMenuKey.MenuDsdAppendData].SharedProps.Visible = fObject.canAppendChild;                    
                    // --
                    mnuMenu.Tools[FMenuKey.MenuDsdImportText].SharedProps.Enabled = ((FData)fObject).fFormat == FFormat.List;
                }
            ...
            )
        {
            ...
            try
            {
                ...
                // Data Set List Load
                // ***
                foreach (FDataSetList fDsl in fScd.fChildDataSetListCollection)
                {
            ...
            )
        {
            ...
            try
            {
                ...
                {
                    foreach (FDataSetList fDsl in ((FSecsDriver)fParent).fChildDataSetListCollection)
                    {
            ...
            )
        {
            ...
            try
            {
                ...
                if (fParent.fObjectType == FObjectType.SecsDriver)
                {
                    fNewChild = ((FSecsDriver)fParent).appendChildDataSetList(new FDataSetList(this.m_fSsmCore.fSsmFileInfo.fSecsDriver));
                }
                else if (fParent.fObjectType == FObjectType.DataSetList)
                {
                    fNewChild = ((FDataSetList)fParent).appendChildDataSet(new FDataSet(this.m_fSsmCore.fSsmFileInfo.fSecsDriver));
                }
                else if (fParent.fObjectType == FObjectType.DataSet)
                {
                    fNewChild = ((FDataSet)fParent).appendChildData(new FData(this.m_fSsmCore.fSsmFileInfo.fSecsDriver));
                }
                else if (fParent.fObjectType == FObjectType.Data)
                {
                    fNewChild = new FData(this.m_fSsmCore.fSsmFileInfo.fSecsDriver);
            ...
            )
        {
            ...
            try
            {
                ...
                if (fParent.fObjectType == FObjectType.SecsDriver)
                {
                    fNewChild = ((FSecsDriver)fParent).insertBeforeChildDataSetList(
                        new FDataSetList(this.m_fSsmCore.fSsmFileInfo.fSecsDriver),
                ...
                else if (fParent.fObjectType == FObjectType.DataSetList)
                {
                    fNewChild = ((FDataSetList)fParent).insertBeforeChildDataSet(
                        new FDataSet(this.m_fSsmCore.fSsmFileInfo.fSecsDriver),
                        (FDataSet)fRefChild
                        );
                }
                else if (fParent.fObjectType == FObjectType.DataSet)
                {
                    fNewChild = ((FDataSet)fParent).insertBeforeChildData(
                        new FData(this.m_fSsmCore.fSsmFileInfo.fSecsDriver),
                        (FData)fRefChild
                        );
                }
                else if (fParent.fObjectType == FObjectType.Data)
            ...
            )
        {
            ...
            try
            {
                ...
                if (fParent.fObjectType == FObjectType.SecsDriver)
                {
                    fNewChild = ((FSecsDriver)fParent).insertAfterChildDataSetList(
                        new FDataSetList(this.m_fSsmCore.fSsmFileInfo.fSecsDriver),
                        (FDataSetList)fRefChild
                        );
                }
            ...
            )
        {
            ...
            try
            {
                ...
                if (fParent.fObjectType == FObjectType.SecsDriver)
                {
                    ...
                    // --
                    ((FSecsDriver)fParent).removeChildDataSetList((FDataSetList[])fChilds);
                }
                else if (fParent.fObjectType == FObjectType.DataSetList)
                {
        ...
        private void procMenuImportText(
            )
        {
            UltraTreeNode tNodeParent = null;
            FData fData = null;
            FData fParent = null;
            FData fNewChild = null;
            // --
            string[] importTexts = null;
            ...
            try
            {
                ...
                using (FImportTextEditor fImportTextEditor = new FImportTextEditor(this.m_fSsmCore))
                {
                    if (fImportTextEditor.ShowDialog(this.m_fSsmCore.fSsmContainer) == System.Windows.Forms.DialogResult.OK)
                    {
                        importText = fImportTextEditor.importText;
                    }
                }
                ...
                // --
                importTexts = importText.Split(new string[] { "\n" }, StringSplitOptions.RemoveEmptyEntries);
                if (importTexts == null || importTexts.Length == 0)
                    return;
                
                // --

                tNodeParent = tvwTree.ActiveNode;
                fParent = (FData)tNodeParent.Tag;
                // --
                if (fParent.hasChild)
                {
                    fData = fParent.fChildDataCollection[0];
                }

                // --
                foreach (string txtItem in importTexts)
                {
                    // --
                    if (fData == null)
                    {
                        fNewChild = new FData(this.m_fSsmCore.fSsmFileInfo.fSecsDriver);
                        fNewChild.fFormat = FFormat.Ascii;
                    }
                    else
                    {
                        fNewChild = fData.forceClone();
                    }

                    // --
                    fNewChild.originalStringValue = txtItem;
                    fNewChild = fParent.appendChildData((FData)fNewChild);
                    // --
                }
        ...
        {
            FDataSetValidation fValidation = null;

            try
            {
                fValidation = (FDataSetValidation)m_fSsmCore.fSsmContainer.getChild(typeof(FDataSetValidation));
                if (fValidation == null)
                {
                    fValidation = new FDataSetValidation(m_fSsmCore);
                    m_fSsmCore.fSsmContainer.showChild(fValidation);
                }
                fValidation.activate();
            }
            catch (Exception ex)
            {
                FDebug.throwException(ex);
            ...
            )
        {
            ...
            try
            {
                ...
                if (fParent.fObjectType == FObjectType.SecsDriver)
                {
                    fChild = ((FSecsDriver)fParent).pasteChildDataSetList();
                }
                else if (fParent.fObjectType == FObjectType.DataSetList)
                {
            ...
            )
        {
            ...
            try
            {
                ...
                fResult = m_fSsmCore.fSsmFileInfo.fSecsDriver.searchDataSetSeries(fBase, searchWord);
                if (fResult == null)
                {
            ...
            )
        {
            ...
            try
            {
                ...
                {
                    pgdProp.selectedObject = new FPropDsl(m_fSsmCore, pgdProp, (FDataSetList)fObject);
                }
                else if (fObject.fObjectType == FObjectType.DataSet)
                {
                    pgdProp.selectedObject = new FPropDts(m_fSsmCore, pgdProp, (FDataSet)fObject);
                }
                else if (fObject.fObjectType == FObjectType.Data)
                {
                    pgdProp.selectedObject = new FPropDat(m_fSsmCore, pgdProp, (FData)fObject);
                }
                ...
                if (fDragDropData.fObject != null)
                {
                    ...
                    if (fRefObject.fObjectType == FObjectType.SecsDriver)
                    {
                        if (fDragDropData.fObject.fObjectType == FObjectType.DataSetList)
                        {
                            ...
                            {
                                cnt = ((FSecsDriver)fRefObject).fChildDataSetListCollection.count;
                                fRefObject = ((FSecsDriver)fRefObject).fChildDataSetListCollection[cnt - 1];
                    ...
                    else if (fRefObject.fObjectType == FObjectType.Data)
                    {
                        ...
                        else if (fDragDropData.fObject.fObjectType == FObjectType.SecsItem)
                        {
                            ...
                            if (((FData)fRefObject).equalsModelingFile(fDragDropData.fObject))
                            {
                                if (((FSecsItem)fDragDropData.fObject).fPattern == FPattern.Fixed)
                                {
                                    if (
                                        ((FData)fRefObject).fPattern == FPattern.Fixed ||
                                        ((FData)fRefObject).fPreviousSibling == null ||
                    ...
                    if (fRefObject.fObjectType == FObjectType.SecsDriver)
                    {
                        if (fDragDropData.fObject.fObjectType == FObjectType.DataSetList)
                        {
                            ...
                            else if (e.Effect == DragDropEffects.Copy)
                            {
                                ...
                                fDragDropData.fObject = ((FSecsDriver)fRefObject).pasteChildDataSetList();
                    ...
                    else if (fRefObject.fObjectType == FObjectType.Data)
                    {
                            ...
                            )
                        {
                            ...
                            if (e.Effect == DragDropEffects.Copy)
                            {
                                ...
                                // --
                                if (fDragDropData.fObject.fObjectType == FObjectType.SecsDriver)
                                {
                                ...
                                {
                                    ((FData)fRefObject).sourceResource = FResourceSourceType.SecsDeviceName;
                                }
                                else if (fDragDropData.fObject.fObjectType == FObjectType.SecsSession)
                                {
                                    ((FData)fRefObject).sourceResource = FResourceSourceType.SecsSessionName;
                                }
                        ...
                        else if (fDragDropData.fObject.fObjectType == FObjectType.SecsItem)
                        {
                            ...
                            if (e.Effect == DragDropEffects.Copy)
                            {
                                ...
                                {
                                    ((FData)fRefObject).fPattern = FPattern.Fixed;
                                    ((FData)fRefObject).fixedLength = ((FSecsItem)fDragDropData.fObject).fixedLength;
                                }
                                else
                                {
                                    ((FData)fRefObject).fPattern = FPattern.Variable;
                                ...
                                if (!((FData)fRefObject).hasChild)
                                {
                                    ((FData)fRefObject).fFormat = ((FSecsItem)fDragDropData.fObject).fFormat;
                                }
                                fAction = FDragDropAction.Set;
                            }
                            else
            ...
            )
        {
            try
            {
                ...
                // --
                if (e.Tool.Key == FMenuKey.MenuDsdValidation)
                {
                    procMenuValidation();
                }
                else if (e.Tool.Key == FMenuKey.MenuDsdExpand)
                {
                    procMenuExpand();
                }
                ...
                else if (e.Tool.Key == FMenuKey.MenuDsdImportText)
                {
                    procMenuImportText();
                }
                else if (
