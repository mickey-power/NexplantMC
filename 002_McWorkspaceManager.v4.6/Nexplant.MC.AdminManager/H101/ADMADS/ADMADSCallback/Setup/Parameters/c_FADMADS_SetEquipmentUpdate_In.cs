/*----------------------------------------------------------------------------------------------------------
--  상기 프로그램에 대한 저작권을 포함한 지적재산권은 (주)미라콤아이앤씨에 있으며, (주)미라콤아이앤씨가
--  명시적으로 허용하지 않은 사용, 복사, 변경, 제3자에의 공개, 배포는 엄격히 금지되며, (주)미라콤아이앤씨의
--  지적재산권 침해에 해당됩니다.
--  (Copyright ⓒ 2014 Miracom Inc. All Rights Reserved | Confidential)
--
--  Program Id      : c_FADMADS_SetEquipmentUpdate_In.cs
--  Creator         : Jungyoul.moon
--  Create Date     : 2016.05.16
--  Description     : <Generated Class File Description>
--  History         : Created by Jungyoul.moon at 2016.05.16
----------------------------------------------------------------------------------------------------------*/
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Nexplant.MC.H101Interface
{
    internal static class FADMADS_SetEquipmentUpdate_In
    {

        //------------------------------------------------------------------------------------------------------------------------

        public const string E_ADMADS_SetEquipmentUpdate_In = "ADMADS_SetEquipmentUpdate_In";

        // --

        public const string A_hLanguage = "hLanguage";
        public const string A_hFactory = "hFactory";
        public const string A_hUserId = "hUserId";
        public const string A_hHostIp = "hHostIp";
        public const string A_hHostName = "hHostName";
        public const string A_hStep = "hStep";

        // --

        public const string D_hLanguage = "";
        public const string D_hFactory = "";
        public const string D_hUserId = "";
        public const string D_hHostIp = "";
        public const string D_hHostName = "";
        public const string D_hStep = "";

        // --

        public static class FEquipment
        {
            public const string E_Equipment = "Equipment";

            // --

            public const string A_Equipment = "Equipment";
            public const string A_EquipmentID = "EquipmentID";
            public const string A_Description = "Description";
            public const string A_EquipmentClass = "EquipmentClass";
            public const string A_DeptSkipValidation = "DeptSkipValidation";
            public const string A_EquipmentType = "EquipmentType";
            public const string A_EquipmentArea = "EquipmentArea";
            public const string A_EquipmentLine = "EquipmentLine";
            public const string A_FmbMonitoring = "FmbMonitoring";
            public const string A_IpAddress = "IpAddress";

            // --

            public const string D_Equipment = "";
            public const string D_EquipmentID = "";
            public const string D_Description = "";
            public const string D_EquipmentClass = "";
            public const string D_DeptSkipValidation = "";
            public const string D_EquipmentType = "";
            public const string D_EquipmentArea = "";
            public const string D_EquipmentLine = "";
            public const string D_FmbMonitoring = "";
            public const string D_IpAddress = "";
        }

        //------------------------------------------------------------------------------------------------------------------------

    }   // Class end
}   // Namespace end
