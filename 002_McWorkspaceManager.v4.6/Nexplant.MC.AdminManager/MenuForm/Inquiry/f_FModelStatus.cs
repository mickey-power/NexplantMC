...
--
--  Program Id      : f_FModelStatus.cs
--  Creator         : baehyun.seo
--  Create Date     : 2012.05.14
--  Description     : FAMate Admin Manager Model Status Form Class 
...
{
    public partial class FModelStatus : Nexplant.MC.Core.FaUIs.FBaseTabChildForm
    {
        ...
        private void designGridOfModelDetail(
            )
        {
            string[] columns = null;
        ...
        private void refreshGridOfModelDetail(
             )
        {
            ...
            try
            {
                ...
                if (txtModel.Text == string.Empty || txtVersion.Text == string.Empty)
                {
                    grdDetail.endUpdate();
                    txtVersion.Focus();
                ...
                fSqlParams.add("mdl_ver", txtVersion.Text);

                // --

                dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Inquiry", "ModelStatus", "SearchModel", fSqlParams, true);
                row = dt.Rows[0];
                ...
                cellValues = new object[] {
                    row[0].ToString(),          // Model
                    row[1].ToString(),          // Description
                    row[2].ToString(),          // Model Type
                    // --
             ...
             )
        {
            ...
            try
            {
                ...
                do
                {
                    dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Inquiry", "ModelStatus", "ListEap", fSqlParams, false, ref nextRowNumber);
                    // --
            ...
            )
        {
            ...
            try
            {
                ...
                cellValues = new object[] {
                    ...
                    string.Format("{0:#,#} MB", int.Parse(maxSecsLogFileSize) / 1024 / 1024),   // Max SECS/PLC/OPC Log File Size (MB)
                    string.Format("{0:#,#} Byte", int.Parse(maxSecsLogFileSize)),               // Max SECS/PLC/OPC Log File Size (Byte)
                    // --
            ...
            )
        {
            ...
            try
            {
                fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_ModelStatus_" + txtModel.Text + "_" + txtVersion.Text + ".xlsx";

                // --

                sfd = new SaveFileDialog();
                sfd.Title = "Export Model Status to Excel";
                ...
                fExcelSht = fExcelExp.addExcelSheet("Model Status");
                ...
                // ***
                // Model Detail Grid Write
                // ***
                rowIndex += 2;
                fExcelSht.writeText("[" + m_fAdmCore.fUIWizard.searchCaption("Model Detail") + "]", rowIndex, 0, m_fAdmCore.fUIWizard.fontName, 9, true);
                // --
                rowIndex += 1;
            ...
            )
        {
            ...
            try
            {
                ...
                // ***
                // Model Status EAP Detail Grid Write
                // ***
                rowIndex += 2;
            ...
            )
        {
            ...
            try
            {
                ...
                fSqlParams = new FSqlParams();
                fSqlParams.add("factory", m_fAdmCore.fOption.factory);
                dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Inquiry", "ModelStatus", "SearchFactory", fSqlParams, true);
            ...
            )
        {
            try
            {
                ...
                txtModel.Tag = modelType;
                txtVersion.Text = modelVersion;
        ...
        #region FModelStatus Form Event Handler

        private void FModelStatus_Load(
            object sender,
            EventArgs e
            )
        {
        ...
        private void FModelStatus_Shown(
            object sender,
            EventArgs e
            )
        {
        ...
        private void FModelStatus_FormClosing(
            object sender, 
        ...
        private void FModelStatus_KeyDown(
            object sender, 
            KeyEventArgs e
            )
        {
            ...
            )
        {
            try
            {
                ...
                if (e.Tool.Key == FMenuKey.MenuMdsRefresh)
                {
                    refresh();
                }
                else if (e.Tool.Key == FMenuKey.MenuMdsExport)
                {
                    export();
                }
            ...
            )
        {
            ...
            try
            {
                if (e.Cell.Row.Index == 3 && e.Cell.Column.Index == 1)
                {
            ...
            )
        {
            ...
            try
            {
                ...
                fDialog = new FModelVersionSelector(m_fAdmCore, txtModel.Text, txtVersion.Text);
                ...
                txtVersion.Text = fDialog.selectedModelVer;
            }
            catch (Exception ex)
            {
                FMessageBox.showError(FConstants.ApplicationName, ex, m_fAdmCore.fWsmCore.fWsmContainer);
            ...
            )
        {
            try
            {
                ...
                {
                    mnuMenu.Tools[FMenuKey.MenuMdsExport].SharedProps.Enabled = false;
                }
