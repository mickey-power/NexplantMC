...
--
--  Program Id      : f_FServerIssueEventHistory.cs
--  Creator         : spike.lee
--  Create Date     : 2017.07.25
--  Description     : FAmate Admin Manager Server Issue Event History Form Class 
--  History         : Created by spike.lee at 2017.07.25
----------------------------------------------------------------------------------------------------------*/
using System;
using System.Data;
...
{
    public partial class FServerIssueEventHistory : Nexplant.MC.Core.FaUIs.FBaseTabChildForm
    {
        //------------------------------------------------------------------------------------------------------------------------
        ...
        public FServerIssueEventHistory(
            )
        {
            InitializeComponent();
        }
            ...
            )
        {
            ...
            try
            {
                ...
                do
                {
                    dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Inquiry", "ServerIssueEventHistory", "ListServer", fSqlParams, false, ref nextRowNumber);
                    // --
            ...
            )
        {
            ...
            try
            {
                ...
                do
                {
                    dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Inquiry", "ServerIssueEventHistory", "SearchServerIssueEventSummary", fSqlParams, false, ref nextRowNumber);
                    // --
                    foreach (DataRow r in dt.Rows)
                    {
                        server = r[0].ToString();
            ...
            )
        {
            ...
            try
            {
                ...
                do
                {
                    using (DataTable dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Inquiry", "ServerIssueEventHistory", "SearchServerIeh", fSqlParams, false, ref nextRowNumber))
                    {
            ...
            )
        {
            ...
            try
            {
                fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_ServerIssueEventHistory" + ".xlsx";

                // --

                sfd = new SaveFileDialog();
                // --                
                sfd.Title = "Export Server Issue Event History to Excel";
                ...
                fExcelSht = fExcelExp.addExcelSheet("Server Issue Event History");
        ...
        #region FServerIssueEventHistory Form Event Handler

        private void FServerIssueEventHistory_Load(
            object sender,
            EventArgs e
            )
        {
        ...
        private void FServerIssueEventHistory_Shown(
            object sender, 
            EventArgs e
            )
        {
        ...
        private void FServerIssueEventHistory_FormClosing(
            object sender, 
        ...
        private void FServerIssueEventHistory_KeyDown(
            object sender, 
            KeyEventArgs e
            )
        {
            ...
            )
        {
            ...
            try
            {
                ...
                if (ret.ChartElementType == ChartElementType.DataPoint)
                {
                    ...
                    refreshGridOfServrHistory(server);
                }
            }
            catch (Exception ex)
            {
            ...
            )
        {
            ...
            try
            {
                ...
                fDialog = new FEventSelector(m_fAdmCore, FEventType.Server.ToString(), txtEvent.Text, false);
