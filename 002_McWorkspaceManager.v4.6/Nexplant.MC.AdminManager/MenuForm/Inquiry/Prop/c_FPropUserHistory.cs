...
--
--  Program Id      : c_FPropUserHistory.cs
--  Creator         : hsshim
--  Create Date     : 2013.01.21
--  Description     : FAMate Admin Manager User History Property Source Object Class 
--  History         : Created by hsshim at 2013.01.21
----------------------------------------------------------------------------------------------------------*/
using System;
...
{
    public class FPropUserHistory : FDynPropCusBase<FAdmCore>
    {
        ...
        private const string CategoryData = "[03] Data Field";
        private const string CategoryComment = "[04] Comments";
            ...
            )
        {
            try
            {
                if (dr != null)
                {
                    ...
                    m_userId             = dr["USER_ID"].ToString();
                    m_systemCode         = dr["SYSTEM"].ToString();
                    m_eventId            = dr["EVENT_ID"].ToString();
                    m_action             = dr["ACTION"].ToString();
                    m_hostIP             = dr["HOST_IP"].ToString();
                    m_hostName           = dr["HOST_NAME"].ToString();
                ...
                base.fTypeDescriptor.properties["HostIP"].attributes.replace(new DisplayNameAttribute("User IP"));
                base.fTypeDescriptor.properties["HostName"].attributes.replace(new DisplayNameAttribute("User PC Name"));
                // --
                ...
                base.fTypeDescriptor.properties["Action"].attributes.replace(new DefaultValueAttribute(m_action));
                base.fTypeDescriptor.properties["SystemCode"].attributes.replace(new DefaultValueAttribute(m_systemCode));
                base.fTypeDescriptor.properties["HostIP"].attributes.replace(new DefaultValueAttribute(m_hostIP));
                base.fTypeDescriptor.properties["HostName"].attributes.replace(new DefaultValueAttribute(m_hostName));
                // --
