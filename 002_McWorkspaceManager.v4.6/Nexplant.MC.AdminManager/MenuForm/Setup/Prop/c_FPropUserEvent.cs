...
--
--  Program Id      : c_FPropUserEvent.cs
--  Creator         : mjkim
--  Create Date     : 2012.11.21
--  Description     : FAMate Admin Manager User Event Property Source Object Class 
--  History         : Created by njkim at 2012.11.21
----------------------------------------------------------------------------------------------------------*/
using System;
...
{
    public class FPropUserEvent : FDynPropCusBase<FAdmCore>
    {

        //------------------------------------------------------------------------------------------------------------------------

        private const string CategoryGeneral = "[01] General";
        private const string CategoryAction = "[02] Action";
        private const string CategorySystemEvent = "[03] System Event";
        private const string CategoryData = "[04] Data";
            ...
            )
        {
            try
            {
                ...
                if (dt != null)
                {
                    ...
                    m_action = dt.Rows[0][2].ToString();
                    m_systemEvent = (FYesNo)Enum.Parse(typeof(FYesNo), dt.Rows[0][3].ToString());
                    for (int i = 0; i < m_dataList.Length; i++)
                    {
                        m_dataList[i] = dt.Rows[0][i + 4].ToString();
                    }
                }
                ...
                base.fTypeDescriptor.properties["Action"].attributes.replace(new ParenthesizePropertyNameAttribute(true));
                ...
                if (!m_tranEnabled)
                {
                    ...
                    base.fTypeDescriptor.properties["Action"].attributes.replace(new ReadOnlyAttribute(true));
