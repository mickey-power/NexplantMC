...
--
--  Program Id      : c_FPropEquipmentArea.cs
--  Creator         : iskim
...
{
    public class FPropEquipmentArea : FDynPropCusBase<FAdmCore>
    {
            ...
            )
        {
            try
            {
                if (dt != null)
                {
                    m_area = dt.Rows[0][0].ToString();
                    m_description = dt.Rows[0][1].ToString();                    
                }

                // --
                
                base.fTypeDescriptor.properties["Area"].attributes.replace(new DisplayNameAttribute("Area"));
                ...
                // 데이터 키 정의
                // ***
                base.fTypeDescriptor.properties["Area"].attributes.replace(new ParenthesizePropertyNameAttribute(true));
