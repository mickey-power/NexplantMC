...
--
--  Program Id      : f_FSecs1ToHsmsConverterHistory.cs
--  Creator         : spike.lee
--  Create Date     : 2017.04.28
--  Description     : FAmate Admin Manager SECS1 To HSMS Converter History Form Class 
...
{
    public partial class FSecs1ToHsmsConverterHistory : Nexplant.MC.Core.FaUIs.FBaseTabChildForm, FIInquiry
    {
        ...
        private DataTable m_dtFs2hcvthis = null;
        private DataTable m_dtFs2hevtdef = null;
        ...
        public FSecs1ToHsmsConverterHistory(
            )
        {
            InitializeComponent();
        }
            ...
            )
        {
            ...
            try
            {
                ...
                // --
                grdList.DisplayLayout.Bands[0].Columns["Time"].CellAppearance.TextHAlign = Infragistics.Win.HAlign.Left;
                ...
                grdList.DisplayLayout.Bands[0].Columns["Comment"].Width = 100;
            ...
            )
        {
            try
            {
                ...
                if (m_dtFs2hcvthis == null)
                {
                    m_dtFs2hcvthis = new DataTable();
                    foreach (string clmName in new string[] { "TRAN_TIME", "HIST_SEQ", "CVT_NAME", "CVT_IP", "EVENT_ID", "UP_DOWN", "SECS1_STATE", "HSMS_STATE", "TRAN_USER_ID" })
                    {
                        // ***
                        ...
                        if (clmName == "HIST_SEQ")
                        {
                            m_dtFs2hcvthis.Columns.Add(clmName, typeof(UInt64));
                        }
                        else
                        {
                            m_dtFs2hcvthis.Columns.Add(clmName, typeof(string));
                        }
                    }
                    // --
                    for (int i = 1; i <= 20; i++)
                    {
                        ...
                        {
                            m_dtFs2hcvthis.Columns.Add(string.Format("{0}_{1}", clmName, i.ToString()), typeof(string));
                        }
                    }
                    // --
                    m_dtFs2hcvthis.Columns.Add("TRAN_COMMENT", typeof(string));
                }
                m_dtFs2hcvthis.Rows.Clear();

                // --

                if (m_dtFs2hevtdef == null)
                {
                    m_dtFs2hevtdef = new DataTable();
                    foreach (string clmName in new string[] { "EVENT_ID", "EVENT_DESC" })
                    {
                        m_dtFs2hevtdef.Columns.Add(clmName, typeof(string));
                    }
                }
                m_dtFs2hevtdef.Rows.Clear();
            ...
            )
        {
            ...
            try
            {
                ...
                // --
                using (DataTable dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Secs1ToHsms", "Secs1ToHsmsConverterHistory", "HasSecs1ToHsmsConverter", fSqlParams, false))
                {
                    if (dt.Rows.Count == 0)
                    {
                        FDebug.throwFException(fUIWizard.generateMessage("E0010", new object[] { "Converter" }));
                    }
                }
            }
            catch (Exception ex)
            ...
            )
        {
            try
            {
                pgdProp.selectedObject = new FPropSecs1ToHsmsConverterHistory(m_fAdmCore, pgdProp);
            }
            catch (Exception ex)
            {
                FDebug.throwException(ex);
            ...
            )
        {
            ...
            try
            {
                ...
                do
                {
                    using (DataTable dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Common", "EventSearch", "SearchSecs1ToHsmsEventAll", fSqlParams, false, ref nextRowNumber))
                    {
                        dt.AsEnumerable().Take(dt.Rows.Count).CopyToDataTable(m_dtFs2hevtdef, LoadOption.OverwriteChanges);
                    }
                }
                ...
                // ***
                // Search SECS1 To HSMS Converter History
                // ***
                foreach (string[] reqTimes in m_lstReqTimes)
                {
                    ...
                    do
                    {
                        using (DataTable dt = FCommon.requestCommonSqlQuery(m_fAdmCore, "Secs1ToHsms", "Secs1ToHsmsConverterHistory", "SearchSecs1ToHsmsConverterHistory", fSqlParams, false, ref nextRowNumber))
                        {
                            dt.AsEnumerable().Take(dt.Rows.Count).CopyToDataTable(m_dtFs2hcvthis, LoadOption.OverwriteChanges);
                        }
                        // --
             ...
             )
        {
            ...
            try
            {
                ...
                m_dtFs2hcvthis.DefaultView.Sort = "HIST_SEQ ASC";
                foreach (DataRow r in m_dtFs2hcvthis.DefaultView.ToTable().Rows)
                {
                    cellValues = new object[] {
                        ...
                        r["CVT_IP"].ToString(),
                        r["UP_DOWN"].ToString(),
                        r["SECS1_STATE"].ToString(),
                        r["HSMS_STATE"].ToString(),
                    ...
                    // --
                    rowIndex = grdList.appendDataRow(generateKey(new string[] { r["CVT_NAME"].ToString(), r["HIST_SEQ"].ToString()} ), cellValues).Index;
                    grdList.Rows[rowIndex].Tag = r;
                    
                    // --

                    m_dtFs2hevtdef.DefaultView.RowFilter = string.Format("EVENT_ID = '{0}'", r["EVENT_ID"].ToString());
                    if (m_dtFs2hevtdef.DefaultView.ToTable().Rows.Count > 0)
                    {
                        grdList.Rows[rowIndex].Cells["Event"].ToolTipText = m_dtFs2hevtdef.DefaultView.ToTable().Rows[0]["EVENT_DESC"].ToString();
                    }
                }
            ...
            )
        {
            ...
            try
            {
                fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "_Secs1ToHsmsConverterHistory.xlsx";

                // --

                sfd = new SaveFileDialog();
                // --
                sfd.Title = "Export SECS1 To HSMS Converter History to Excel";
                ...
                fExcelSht = fExcelExp.addExcelSheet("SECS1 To HSMS Converter History");
                ...
                fExcelSht.writeCondition(lblConverter.Text, txtConverter.Text, rowIndex, 4);

                // --

                // ***
                // SECS1 To HSMS Converter History Grid Write
                // ***
                rowIndex += 2;
                fExcelSht.writeText("[" + m_fAdmCore.fUIWizard.searchCaption("SECS1 To HSMS Converter History") + "]", rowIndex, 0, m_fAdmCore.fUIWizard.fontName, 9, true);
                // --
                rowIndex += 1;
        ...
        #region FSecs1ToHsmsConverterHistory Form Event Handler

        private void FSecs1ToHsmsConverterHistory_Load(
            object sender, 
            EventArgs e
            )
        {
        ...
        private void FSecs1ToHsmsConverterHistory_FormClosing(
            object sender, 
        ...
        private void FSecs1ToHsmsConverterHistory_Shown(
            object sender, 
            EventArgs e
            )
        {
        ...
        private void FSecs1ToHsmsConverterHistory_KeyDown(
            object sender, 
            KeyEventArgs e
            )
        {
            ...
            )
        {
            ...
            try
            {
                ...
                fDialog = new FSecs1ToHsmsConverterSelector(m_fAdmCore, txtConverter.Text, "");
            ...
            )
        {
            try
            {
                ...
                pgdProp.selectedObject = new FPropSecs1ToHsmsConverterHistory(m_fAdmCore, pgdProp, (DataRow)grdList.ActiveRow.Tag);
            }
            catch (Exception ex)
            {
                FMessageBox.showError(FConstants.ApplicationName, ex, m_fAdmCore.fWsmCore.fWsmContainer);
            ...
            )
        {
            try
            {
                ...
                {
                    m_fInqProgress.changedProgress(e.ProgressPercentage, m_dtFs2hcvthis.Rows.Count);
                }
            }
            catch (Exception ex)
            {
