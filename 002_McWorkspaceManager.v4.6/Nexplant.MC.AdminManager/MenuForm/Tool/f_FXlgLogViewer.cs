...
--
--  Program Id      : f_FXlgLogViewer.cs
--  Creator         : mjkim
--  Create Date     : 2013.01.11
--  Description     : FAMate Admin Manager XLG Log Viewer Form Class 
...
{
    public partial class FXlgLogViewer : Nexplant.MC.Core.FaUIs.FBaseTabChildForm
    {
            ...
            )
        {
            try
            {
                ...
                mnuMenu.Tools[FMenuKey.MenuXlvRefresh].SharedProps.Enabled = m_fileName == string.Empty ? false : true;
                // --
            ...
            )
        {
            ...
            try
            {
                preFontName = ((FontListTool)mnuMenu.Tools[FMenuKey.MenuXlvFontName]).Value.ToString();
                // -- 
            ...
            )
        {
            try
            {
                ...
                txtFileName.Text = "Convert To XLG";
                controlMenu();
            }
            catch (Exception ex)
            {
        ...
        #region FXlgLogViewer Form Event Handler

        private void FXlgLogViewer_Load(
            object sender,
            EventArgs e
            )
        {
            try
            {
                ...
                ((FontListTool)mnuMenu.Tools[FMenuKey.MenuXlvFontName]).Text = m_fAdmCore.fOption.fontName;
                numFontSize.Value = m_fAdmCore.fOption.fontSize;

                // --

                txtLog.Appearance.FontData.Name = ((FontListTool)mnuMenu.Tools[FMenuKey.MenuXlvFontName]).Value.ToString();
                txtLog.Appearance.FontData.SizeInPoints = float.Parse(numFontSize.Value.ToString());
        ...
        private void FXlgLogViewer_Shown(
            object sender, 
            EventArgs e
            )
        {
        ...
        private void FXlgLogViewer_FormClosing(
            object sender, 
        ...
        private void FXlgLogViewer_KeyDown(
            object sender, 
            KeyEventArgs e
            )
        {
            ...
            )
        {
            try
            {
                ...
                else if (e.Tool.Key == FMenuKey.MenuXlvPrevious)
                {
                    continueLogFile(FLogContinuity.Previous);
                }
                else if (e.Tool.Key == FMenuKey.MenuXlvNext)
                {
            ...
            )
        {
            try
            {
                ...
                if (e.Tool.Key == FMenuKey.MenuXlvFontName)
                {
            ...
            )
        {
            try
            {
                ...
                if (e.Tool.Key == FMenuKey.MenuXlvFontName)
                {
                    ...
                    {
                        ((FontListTool)mnuMenu.Tools[FMenuKey.MenuXlvFontName]).Value = m_fAdmCore.fOption.fontName;
