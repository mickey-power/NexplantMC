...
--  Description     : FAMate OPC Modeler Replace Name Dialog Form Class
...
namespace Nexplant.MC.OpcModeler
{
    public partial class FReplaceNameDialog : Nexplant.MC.Core.FaUIs.FBaseStandardDialogForm, IDisposable
    {
            ...
            )
        {
            ...
            try
            {
                caption = m_fOpmCore.fUIWizard.searchCaption(this.Text);
                this.Text = caption;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (txtFindWhat.Text == string.Empty)
                {
                    FDebug.throwFException(
                        m_fOpmCore.fWsmCore.fUIWizard.generateMessage("E0004", new object[] { "Find What" })
                        );
                }
